...
void GraphSimplify::HHSimplify(const Graph& G, const std::vector<double>& prob, const std::unordered_set<int>& srcs, int tgt){
  int mold = G.m;
  int nold = G.n;
  std::vector<std::unordered_map<int, int>> adjs(nold+1);
  std::vector<double> tmpi(prob);
  bool updated = false;
  auto srcsnew0 = srcs;
  tgtnew = tgt;
  ...
  do{
    ...
    for(int k=1; k<=nold; ++k){
      if(adjs[k].size() == 1){
        ...
        double newprob = tmpi[kitr1->second] * tmpi[kitr2->second];
        int kk1 = kitr1->first;
...
void GraphSimplify::LWSimplify(const Graph& G, const std::vector<double>& prob, const std::unordered_set<int>& srcs){
  int mold = G.m;
  int nold = G.n;
  std::vector<std::unordered_map<int, int>> adjs(nold+1);
  bool updated = false;
